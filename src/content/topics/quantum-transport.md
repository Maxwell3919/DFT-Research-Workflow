...
For a [SIESTA](/DFT-Research-Workflow/tools/siesta/) TranSIESTA/TBtrans route, retain separate electrode and device `.fdf` inputs, pseudopotential and basis identities, electrode Hamiltonian/surface data, the central-region Hamiltonian such as a `.TSHS` record, stdout/stderr, and the TBtrans input and raw transmission/current tables. Open the electrode, principal layers, matching planes, and central region in [VESTA](/DFT-Research-Workflow/tools/vesta/) before launch; after launch, plot lead bands, $T(E,k_\perp)$, projected bulk continuum, potential, and terminal-current residual together. If the lead surface Green function fails, density or potential is not bulk-like at matching planes, left and right currents disagree, a narrow resonance moves with energy grid, or the result changes with central length, repair the open-system parent before interpreting an eigenchannel or I--V curve.
