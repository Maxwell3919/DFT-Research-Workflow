...
Start with a full-zone gap table and the symmetry/SOC/magnetic record from the parent calculation, then retain the exact [Wannier90](/DFT-Research-Workflow/tools/wannier90/) or other reduced Hamiltonian and its direct-parent comparison. For a Wilson-loop or Z2Pack-style run, preserve the loop family, mesh, raw WCC phases, convergence report, and plotted flow; for a WannierTools-style boundary calculation, preserve the control input, `wannier90_hr.dat`, orientation/termination, bulk-projected spectrum, layer localization, and raw surface or edge spectral data. Open browser-based symmetry results only as an independent record tied to the same structure and Hamiltonian assumptions. If a full-zone gap closes, an invariant changes with mesh/subspace/symmetry tolerance, or a boundary crossing lacks bulk-band connectivity or localization, stop the classification or boundary claim instead of selecting the most topological-looking plot.
