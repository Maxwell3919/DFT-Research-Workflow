...
For a human audit, open the retained trajectory in [ASE](/DFT-Research-Workflow/tools/ase/) beside the collective-variable time series, then inspect the [PLUMED](/DFT-Research-Workflow/tools/plumed/) input and its `COLVAR`, bias, and, when produced, `HILLS` or window-histogram records before opening a reconstructed free-energy curve. Keep one manifest-like table of window centre, force constant, temperature, retained interval, frame stride, effective sample count, and neighbour overlap; this is a reading aid, not a new workflow schema. If adjacent windows do not overlap, reweighting is dominated by a few frames, replicas remain in their starting basin, or the result changes with blocks/bins, stop the free-energy claim and add independent support or revise the collective variable instead of smoothing across the gap.
