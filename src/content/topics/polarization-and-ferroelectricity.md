...
Keep every interpolated structure as a numbered file and retain, for the same index, the electronic output, raw polarization representative, cell, energy, and band-gap check. Step through the structures in [VESTA](/DFT-Research-Workflow/tools/vesta/) or [ASE](/DFT-Research-Workflow/tools/ase/) before unwrapping the numerical series. A [VASP](/DFT-Research-Workflow/tools/vasp/) route should preserve the input and complete polarization-bearing output for every image; a [Quantum ESPRESSO](/DFT-Research-Workflow/tools/quantum-espresso/) finite-field or polarization route needs the same image-by-image state and branch lineage. If an atom changes identity, the cell basis changes unexpectedly, the gap closes, or the electronic state switches, stop at that image: branch arithmetic cannot repair a broken adiabatic path.
