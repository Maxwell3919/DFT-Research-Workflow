...
Before launch, place the initial structure, calculator input, velocity or restart record, thermostat/barostat settings, and intended sampling log in one run directory. During the run, tail the main stdout and stderr while opening the growing XYZ, trajectory, or code-native coordinate file in [ASE](/DFT-Research-Workflow/tools/ase/) or another trajectory viewer; mark the exact frame and step for every structural anomaly. [Quantum ESPRESSO](/DFT-Research-Workflow/tools/quantum-espresso/) and [CP2K](/DFT-Research-Workflow/tools/cp2k/) expose different dynamics controls, so retain their version-matched input and restart objects rather than translating only an ensemble label. If SCF failures, force spikes, energy drift, thermostat oscillation, cell collapse, or restart discontinuity appears, preserve that interval and repair the numerical or ensemble setup before discarding frames or calling the remaining segment production.
