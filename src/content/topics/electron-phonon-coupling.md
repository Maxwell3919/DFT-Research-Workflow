...
One executable starting point is the [Quantum ESPRESSO](/DFT-Research-Workflow/tools/quantum-espresso/) plus [Wannier90](/DFT-Research-Workflow/tools/wannier90/) plus [EPW](/DFT-Research-Workflow/tools/epw/) chain. Before writing an EPW input, place the accepted SCF and uniform-mesh NSCF inputs/outputs, the matching `prefix.save` state, the complete `ph.x` dynamical and perturbation data, and the Wannier `.win`, `.amn`, `.mmn`, `.eig`, `.wout`, and checkpoint records in one ancestry ledger. In the terminal, first confirm that every parent stage has its own stdout and stderr and that every required irreducible q point completed; then compare direct and interpolated bands, phonons, and selected matrix elements before opening the dense-grid $\alpha^2F$, linewidth, or scattering tables. A missing q point, unresolved imaginary mode, mismatched save tree, or failed Wannier window is a stop condition, not an interpolation detail.
