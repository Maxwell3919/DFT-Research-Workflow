...
A concrete working directory should make the comparison visible before any fit: retain the accepted reference cell, one named structure file for every positive and negative strain, and a table containing the deformation matrix, energy, all printed stress components, residual forces, and relaxation status. Open the cells in [VESTA](/DFT-Research-Workflow/tools/vesta/) or [ASE](/DFT-Research-Workflow/tools/ase/), then read the matching output from the selected calculator rather than copying a modulus from a summary line. [VASP](/DFT-Research-Workflow/tools/vasp/) users can inspect the input cell and the stress records in `OUTCAR` or `vaspout.h5`; other codes expose equivalent cell, energy, force, and stress objects. If the positive and negative branches are asymmetric, an internal relaxation changes state, or residuals grow when the fit window is narrowed, stop the fit and repair that row or the reference state instead of averaging the discrepancy away.
