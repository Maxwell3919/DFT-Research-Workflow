...
if rg -q '^exit=0$' "$case_dir/output/chromium.stdout"; then pass 'G1 Chromium headless command exited successfully'; else fail 'G1 missing successful Chromium exit marker'; fi
if file "$case_dir/figures/chromium-headless-walkthrough.png" | rg -q 'PNG image data, 1280 x 720'; then pass 'G3 expected 1280 x 720 PNG artifact exists'; else fail 'G3 PNG dimensions or type differ'; fi
if ! rg -q '/home/talos/work/DFT-RW-wt-gui' "$case_dir/output/chromium.stderr"; then pass 'G0 public stderr contains no local case path'; else fail 'G0 public stderr contains a local path'; fi
