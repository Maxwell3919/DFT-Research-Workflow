...
ROOT = Path(__file__).resolve().parent; SRC=ROOT/'source'/'silicon-cod-9013102.cif'; SAN=ROOT/'source'/'silicon-cod-9013102.sanitization.json'; OUT=ROOT/'output'; DER=ROOT/'derived'; FIG=ROOT/'figures'; INP=ROOT/'input'
...
def main():
 ...
 report={'source':facts(a),'primitive':facts(prim),'standardized_conventional':facts(conv),'supercell':facts(sc),'symmetry_tolerance_sweep':sweep,'reciprocal_path':reciprocal,'software':{'python':f'{sys.version_info.major}.{sys.version_info.minor}','ase':__import__('ase').__version__,'spglib':spglib.__version__,'seekpath':seekpath.__version__,'pymatgen_core':Structure.__module__.split('.')[0]}}
