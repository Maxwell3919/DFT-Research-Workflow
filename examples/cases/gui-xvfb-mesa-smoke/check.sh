...
elif grep -q '^exit=0$' "$case_dir/output/docker-build.stdout" && grep -q '^exit=0$' "$case_dir/output/docker-run.stdout"; then
  pass 'G1 Docker build and X11 command exited successfully'
  if grep -q '^xdpyinfo=PASS$' "$case_dir/output/x11-smoke.txt" && grep -q '^software_renderer=PASS llvmpipe$' "$case_dir/output/x11-smoke.txt"; then pass 'G1 Xvfb display and Mesa llvmpipe passed'; else fail 'G1 Xvfb or llvmpipe marker missing'; fi
  if grep -Eq '^window_detection=PASS id=[0-9]+$' "$case_dir/output/x11-smoke.txt"; then pass 'G3 xdotool found the real xmessage window'; else fail 'G3 xdotool window detection marker missing'; fi
  if file "$case_dir/figures/xvfb-mesa-window.png" | grep -q 'PNG image data, 1280 x 720'; then pass 'G3 1280 x 720 screenshot exists'; else fail 'G3 screenshot type or dimensions differ'; fi
