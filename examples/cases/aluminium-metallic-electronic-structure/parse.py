...
def main() -> None:
    ...
    plt.close(figure)
    artifacts = [artifact("SCF input", "input/scf.in"), artifact("NSCF input", "input/nscf.in"), artifact("bands input", "input/bands.in"), artifact("SCF stdout", "output/scf.out"), artifact("SCF stderr", "output/scf.err"), artifact("NSCF stdout", "output/nscf-full.out"), artifact("NSCF stderr", "output/nscf-full.err"), artifact("bands stdout", "output/bands.out"), artifact("bands stderr", "output/bands.err"), artifact("raw-derived mesh table", "derived/al-mesh.csv"), artifact("raw-derived path table", "derived/al-path.csv"), artifact("raw-derived summary", "derived/captured-run-summary.json"), artifact("raw-derived PNG", "figures/raw-nscf-band2-sampling.png")]
