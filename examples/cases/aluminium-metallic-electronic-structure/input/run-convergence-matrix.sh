#!/usr/bin/env bash
# Execute the prepared SCF k-mesh/smearing screen only in a new external root.
set -euo pipefail

case_dir="$(cd "$(dirname "$0")/.." && pwd)"
for required_var in QE_PW QE_PSEUDO_DIR RUN_OUTPUT_ROOT; do
  if [[ -z "${!required_var:-}" ]]; then
    printf 'FAIL set %s before executing the external convergence matrix\n' "$required_var" >&2
    exit 2
  fi
done
...
python3 "$case_dir/input/generate_convergence_inputs.py" --output-dir "$RUN_OUTPUT_ROOT"
...
python3 "$case_dir/input/parse_convergence.py" --run-dir "$runtime_dir" --out-dir "$runtime_dir/assessment"
printf 'PASS external exploratory convergence screen written under %s; G4 remains NOT TESTED\n' "$runtime_dir"
