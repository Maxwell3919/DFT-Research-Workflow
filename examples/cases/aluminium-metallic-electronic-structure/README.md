...
`run.sh` reproduces the execution ordering — SCF, full-zone NSCF, `dos.x`, then
the path calculation — only in a new caller-selected external directory, so
...
mathematical sign/residual of a bounded quadratic interpolation. Its first
parser attempt correctly blocked an erroneous hand-entered volume declaration;
