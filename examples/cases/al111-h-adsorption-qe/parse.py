...
def extract_qe():
    ...
    for role, rel in [
        ...
        ('pseudopotential-identity-hashes', 'output/qe-run/pseudo.sha256'), ('historical-recovery-record', 'output/qe-run/execution-record.json'), ('qe-stdout', 'output/qe-run/pw.stdout'),
        ('qe-stderr', 'output/qe-run/pw.stderr'), ('terminal-failure-summary', 'derived/qe-scf-summary.json'), ('geometry-table', 'derived/geometry-summary.csv'), ('constructed-geometry-figure', 'figures/al111-h-fcc-xz.png'),
    ]:
    ...
    manifest = {
        ...
        'sources': sources, 'commands': [{'stage': 'construct', 'command': 'python3 parse.py --prepare', 'exit_code': 0}, {'stage': 'historical-recovery', 'command': 'python3 recover_execution.py --evidence-dir output/qe-run --origin historical-recovery/generated-from-raw --job-id 34 --host-label Talos --launcher "srun --ntasks=1 --cpus-per-task=1" --walltime-boundary 00:10:00', 'exit_code': 0}, {'stage': 'terminal-extract', 'command': 'bash extract.sh', 'exit_code': 0}],
        'artifacts': artifacts,
