...
def main():
 ...
 unit=graphene(formula='C2',a=2.46,vacuum=12.0); unit.center(vacuum=12.0,axis=2); write(SRC/'graphene-unit.xyz',unit)
 slab=unit.repeat((2,2,1)); slab.pbc=(True,True,False); write(OUT/'graphene-2x2-vacuum.xyz',slab); write(INP/'POSCAR.graphene-2x2-vacuum',slab,format='vasp',direct=True,vasp5=True)
 decorated=slab.copy(); top=float(decorated.positions[:,2].max()); add_adsorbate(decorated,'H',height=1.50,position=(0.0,0.0)); decorated.center(vacuum=12.0,axis=2); decorated.info.pop('adsorbate_info',None); write(OUT/'graphene-2x2-H-ontop.xyz',decorated); write(OUT/'POSCAR.graphene-2x2-H-ontop',decorated,format='vasp',direct=True,vasp5=True)
 ...
 summary={'unit':f(unit),'vacuum_model':f(slab),'adsorbate_model':f(decorated),'construction':{'repeat':[2,2,1],'vacuum_target_ang':12.0,'initial_H_height_ang':1.5,'H_above_highest_C_ang':round(float(decorated.positions[hidx,2]-substrate.max()),8),'empty_cell_length_ang':round(vac,8),'kz_guidance':'This structural case has pbc z=false; it does not select a k-mesh.'},'software':{'python':sys.version.split()[0],'ase':__import__('ase').__version__}}
