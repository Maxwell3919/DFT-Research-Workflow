...
try {
  ...
  if (parent.cards !== 0 || parent.scripts !== 0 || parent.overflow) throw new Error(`defect parent mismatch: ${JSON.stringify(parent)}`);
  for (const guide of guides) if (parent.links.includes(`${base}${guide.route}`)) throw new Error(`defect parent promotes synthetic-only route ${guide.route}`);
  const desktop = [];
  ...
  console.log('Defect-guide smoke passed: synthetic-only routes remain directly readable but are absent from the primary parent list, have no filler figures, retain pinned Python metadata, and pass 1440px/390px plus no-JavaScript checks.');
} finally {
  await browser.close();
}
