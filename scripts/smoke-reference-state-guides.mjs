...
try {
  ...
  if (parent.cards !== 4) throw new Error(`reference-state parent has ${parent.cards} practical cards instead of 4`);
  for (const guide of guides) if (!parent.links.includes(`${base}${guide.route}`)) throw new Error(`reference-state parent is missing ${guide.route}`);
  if (parent.scripts !== 0 || parent.overflow) throw new Error('reference-state parent interface is not static or overflows');
  ...
  if (artifactDirectory) {
    ...
    await page.goto(`${base}${guides[2].route}`, { waitUntil: 'domcontentloaded' });
    ...
    await writeFile(join(artifactDirectory, 'reference-state-guides-summary.json'), `${JSON.stringify({
      ...
      mobile_width: 390,
      no_javascript: true,
      deterministic_metadata_and_comparison_fixtures_only: true,
    }, null, 2)}\n`);
  }

  console.log('Reference-state guide smoke passed: parent cards, 4 static guides, pinned Python metadata, sources, original media, 1440px/390px layout, and no-JavaScript reading.');
} finally {
  await browser.close();
}
