...
async function inspect(page, expectedWidth) {
  ...
  if (result.copyEnhancements !== 1 || result.unexpectedScripts !== 0) {
    throw new Error(`optimization overview exposes ${result.copyEnhancements} copy enhancements and ${result.unexpectedScripts} unexpected scripts`);
  }
  if (result.copyButtons !== result.copyableBlocks) {
    throw new Error(`optimization overview renders ${result.copyButtons} Copy controls for ${result.copyableBlocks} copyable code blocks`);
  }
  ...
  if (result.cards !== 3) throw new Error(`optimization overview exposes ${result.cards} non-synthetic practical cards instead of 3`);
...
try {
  ...
  console.log(`Reviewed optimization smoke passed: common-not-universal route and four distinct gates, ${desktop.headings.length} natural sections, 3 non-synthetic practical cards, rendered sources, bounded Copy enhancement, 1440px/390px no-overflow, no-JavaScript reading, and a separate reference-ground-state boundary.`);
} finally {
  await browser.close();
}
