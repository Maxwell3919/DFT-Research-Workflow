...
async function inspect(page, expectedWidth) {
  ...
  if (result.headings.length !== 20) throw new Error(`reference-state overview has ${result.headings.length} sections instead of 20`);
...
try {
  ...
  console.log(`Reviewed reference-state smoke passed: ${desktop.headings.length} natural sections, 4 practical cards, rendered sources, 1440px/390px no-overflow, no-JavaScript reading, and bounded candidate-state language.`);
} finally {
  await browser.close();
}
