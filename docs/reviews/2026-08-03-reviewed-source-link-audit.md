...
The machine-readable authority is `sources/reviewed-links.json`. The current inventory covers twelve reviewed article/review pairs:
...
The manifest declares 114 unique HTTPS destinations. Some sources legitimately support more than one topic. Each article/review pair must contain exactly its declared source set, while the network layer requests each unique URL once and records every topic that reuses it.
...
The Adsorption Energies authoring environment also completed a fresh 114/114 network audit on 2026-08-04. The new CMR project page, binary database, licence page, adsorption DOI resolvers, and official ASE and pymatgen pages all satisfied the declared semantics. This local observation is separate from the required Hosted CI evidence.
